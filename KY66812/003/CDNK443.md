...
```
// CDNK443编译后运行转后台+自启动0
// 时间: 2025-10-16
// 版本: 1.0
// 说明: 本程序用于编译后运行转后台+自启动
// 使用方法: 
// 1. 编译  gcc CDNK443.c -o CDNK443 -lnetfilter_queue -lpthread -lcurl
// 3. 添加自启动
// 4. 运行程序 ./CDNK443 -q 443 -w 37 -c 0
// 5. 程序运行后会自动转后台+自启动
...
// 9. ./CDNK443 -u 卸载服务
// 10. ./CDNK443 -h 显示帮助信息
// 11. 添加规则: iptables -I OUTPUT -p tcp --sport 443 -j NFQUEUE --queue-num 443 --queue-bypass
...
static int parse_arguments(int argc, char **argv) {
	...
	while ((opt = getopt(argc, argv, "q:w:c:hu")) != -1) {
		...
		case 'u':
			// 卸载服务
			printf("正在卸载CDNK443服务...\n");
			system("sudo systemctl stop CDNK443.service");
			system("sudo systemctl disable CDNK443.service");
			system("sudo rm /etc/systemd/system/CDNK443.service");
			system("pkill -f CDNK443");
			printf("CDNK443卸载成功\n");
			exit(0);
			break;
		case 'h':
			...
			printf("  -u                  卸载CDNK443服务\n");
			printf("  -h                  显示此帮助信息\n\n");
			printf("运行要求:\n");
			printf("  1. 必须以root权限运行\n");
			printf("  2. 需要加载netfilter_queue内核模块\n     modprobe nfnetlink_queue\n");
			printf("示例:\n");
			printf("  %s -q 443 -w 37 -c 0\n", argv[0]);
			printf("  %s -u  # 卸载服务\n\n", argv[0]);
            printf("添加规则示例:\n");
            printf("  iptables -I OUTPUT -p tcp --sport 443 -j NFQUEUE --queue-num 443 --queue-bypass\n\n");
            printf("  技术支持：@mikeuse\n\n");
...
static int create_systemd_service(const char *program_path, int queue_num, int window_size, int confusion_times) {
    ...
    snprintf(service_content, sizeof(service_content),
        "[Unit]\n"
        "Description=CDNK443 TCP Window Size Modifier\n"
    ...
    // 写入服务文件
    service_file = fopen("/etc/systemd/system/CDNK443.service", "w");
    if (!service_file) {
    ...
    fclose(service_file);
    printf("systemd服务文件创建成功: /etc/systemd/system/CDNK443.service\n");
    return 0;
}
...
static int install_and_enable_service() {
    ...
    // 启用服务（开机自启动）
    printf("正在启用CDNK443服务和开机自启动...\n");
    result = system("systemctl enable CDNK443.service");
    if (result != 0) {
        printf("警告: systemctl enable CDNK443.service 失败\n");
        return -1;
    }

    // 启动服务（非阻塞，避免长时间等待）
    printf("正在启动CDNK443服务...\n");
    result = system("systemctl start CDNK443.service --no-block");
    if (result != 0) {
        printf("警告: systemctl start CDNK443.service 失败\n");
        return -1;
    }
    
    printf("CDNK443服务已成功配置为开机自启动\n");
    printf("\n您可以使用以下命令管理服务:\n");
    printf("  启动服务: systemctl start CDNK443.service\n");
    printf("  停止服务: systemctl stop CDNK443.service\n");
    printf("  查看状态: systemctl status CDNK443.service\n");
    printf("  禁用自启: systemctl disable CDNK443.service\n");
    printf("  查看进程: ps aux | grep -v grep | grep CDNK443\n");
    printf("  禁用自启: pkill -f CDNK443\n");
...
static int check_license() {
    ...
    // 授权密钥
    const char *license_key = "CW6V-B45Z-KSXB-73H7";
    const char *url_base = "http://api.5205230.xyz/verify_key_and_date.php";
    char url[512];
    
    snprintf(url, sizeof(url), "%s?license_key=%s", url_base, license_key);
    ...
    // 检查返回值
    if (chunk.memory && strstr(chunk.memory, "授权成功")) {
        free(chunk.memory);
        return 1;
    } else if (chunk.memory && strstr(chunk.memory, "授权到期")) {
        printf("\nNot Running1\n\n");
        free(chunk.memory);
        return 0;
    } else if (chunk.memory && strstr(chunk.memory, "未授权")) {
        printf("\nNot Running2\n\n");
        free(chunk.memory);
        return 0;
    } else {
